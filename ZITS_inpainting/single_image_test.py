...
import numpy as np
import torch
from ZITS_inpainting.src.lsm_hawp.detector import WireframeDetector
from ZITS_inpainting.src.FTR_trainer import ZITS
from ZITS_inpainting.src.config import Config
from skimage.color import rgb2gray
import torchvision.transforms.functional as FF
import torch.nn.functional as F
from skimage.feature import canny
import skimage
from ZITS_inpainting.src.utils import stitch_images, SampleEdgeLineLogits
import metricas
...
def test(model, wf, img_path, mask_path, save_path, valid_th, sigma256=3.0):
    ...
    images.save(save_path + '/' + "res_" + os.path.basename(img_path))
...
def controler(args):
    ...
    wf = wf.to(0)
    wf.load_state_dict(torch.load('./modelos/ZITS/best_lsm_hawp.pth', map_location='cpu')['model'])
    wf.eval()


    global_path_images = os.path.join(args.path_images) 
    global_path_masks = os.path.join(args.path_masks) 
    nombres = os.listdir(global_path_images)
    print("Realizando prediciones")
    for name in tqdm(nombres):
        try:
            os.mkdir(os.path.join(args.save_path, name))
        except:
            pass
        frames = glob.glob(os.path.join(global_path_images, name, "*.jpg"))
        frames.sort()
        if args.mask_type == "Static":
            masks = glob.glob(os.path.join(global_path_masks, name, "*.jpg")) 
            masks.sort()
        else:
            masks = glob.glob(os.path.join(global_path_masks, name, "*.png")) 
            masks.sort()
        for i in range(len(frames)):
            test(model, wf, frames[i], masks[i], os.path.join(args.save_path, name), 0.85, sigma256=3.0)
